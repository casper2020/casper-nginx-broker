/**
 * @file ngx_http_casper_broker_cdn_module.cc
 *
 ...
 */
static ngx_command_t ngx_http_casper_broker_cdn_module_commands[] = {
    {
        ngx_string("nginx_casper_broker_cdn"),                          /* directive name */
        ...
        offsetof(ngx_http_casper_broker_cdn_module_loc_conf_t, enable), /* ... on the 'enable' element */
        NULL
    },
    /* log proposes */
    {
        ngx_string("nginx_casper_broker_cdn_log_token"),
        NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1,
        ngx_conf_set_str_slot,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_http_casper_broker_cdn_module_loc_conf_t, log_token),
        NULL
    },
    /* jwt */
    {
        ngx_string("nginx_casper_broker_cdn_jwt_prefix"),                 /* directive name */
        NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1,
        ngx_conf_set_str_slot,
        NGX_HTTP_LOC_CONF_OFFSET,
        offsetof(ngx_http_casper_broker_cdn_module_loc_conf_t, jwt_prefix),
        NULL
    },
    /* closure */
    ngx_null_command
 ...
 */
static ngx_http_module_t ngx_http_casper_broker_cdn_module_ctx = {
    NULL,                                               /* preconfiguration              */
    ngx_http_casper_broker_cdn_module_filter_init,      /* postconfiguration             */
    NULL,                                               /* create main configuration     */
    NULL,                                               /* init main configuration       */
    NULL,                                               /* create server configuration   */
    NULL,                                               /* merge server configuration    */
    ngx_http_casper_broker_cdn_module_create_loc_conf,  /* create location configuration */
    ngx_http_casper_broker_cdn_module_merge_loc_conf    /* merge location configuration  */
};

/**
 * @brief The casper-nginx-broker 'cdn' module setup data.
 */
ngx_module_t ngx_http_casper_broker_cdn_module = {
    NGX_MODULE_V1,
    &ngx_http_casper_broker_cdn_module_ctx,     /* module context    */
    ngx_http_casper_broker_cdn_module_commands, /* module directives */
...
static void* ngx_http_casper_broker_cdn_module_create_loc_conf (ngx_conf_t* a_cf)
{
    ngx_http_casper_broker_cdn_module_loc_conf_t* conf =
        (ngx_http_casper_broker_cdn_module_loc_conf_t*) ngx_pcalloc(a_cf->pool, sizeof(ngx_http_casper_broker_cdn_module_loc_conf_t));
    if ( NULL == conf ) {
    ...
    conf->jwt_prefix = ngx_null_string;
    return conf;
}
...
static char* ngx_http_casper_broker_cdn_module_merge_loc_conf (ngx_conf_t* a_cf, void* a_parent, void* a_child)
{
    ngx_http_casper_broker_cdn_module_loc_conf_t* prev = (ngx_http_casper_broker_cdn_module_loc_conf_t*) a_parent;
    ngx_http_casper_broker_cdn_module_loc_conf_t* conf = (ngx_http_casper_broker_cdn_module_loc_conf_t*) a_child;

    ngx_conf_merge_value     (conf->enable     , prev->enable     ,            0 ); /* 0 - disabled */
    ngx_conf_merge_str_value (conf->log_token  , prev->log_token  , "cdn_module" );
    ngx_conf_merge_str_value (conf->jwt_prefix , prev->jwt_prefix ,           "" ); //
...
static ngx_int_t ngx_http_casper_broker_cdn_module_filter_init (ngx_conf_t* a_cf)
{
     ...
     */
    const ngx_int_t rv = NGX_BROKER_MODULE_INSTALL_REWRITE_HANDLER(ngx_http_casper_broker_cdn_module_rewrite_handler);
    if ( NGX_OK != rv ) {
        return rv;
    }
...
ngx_int_t ngx_http_casper_broker_cdn_module_content_handler (ngx_http_request_t* a_r)
{
     ...
     */   
    NGX_BROKER_MODULE_CONTENT_HANDLER_BARRIER(a_r, ngx_http_casper_broker_cdn_module, ngx_http_casper_broker_cdn_module_loc_conf_t,
                                              "cdn_module");
    /*
     * This module is enabled.
     */    
    ngx::casper::broker::cdn::Module* module = (ngx::casper::broker::cdn::Module*) ngx_http_get_module_ctx(a_r, ngx_http_casper_broker_cdn_module);
    if ( nullptr == module ) {
        // ... wtf?
        return NGX_ERROR;
    }
...
ngx_int_t ngx_http_casper_broker_cdn_module_rewrite_handler (ngx_http_request_t* a_r)
{
     ...
     * Check if module is enabled and the request can be handled here.
     */
    NGX_BROKER_MODULE_REWRITE_HANDLER_BARRIER(a_r, ngx_http_casper_broker_cdn_module, ngx_http_casper_broker_cdn_module_loc_conf_t,
                                              "cdn_module");
    
    const auto tackle_response = [] (ngx_http_request_t* a_r, ngx::casper::broker::cdn::Module* a_module) -> ngx_int_t {
        //
        if ( true == a_module->SynchronousResponse() ) {
            // ... synchronous response ...
            if ( true == a_module->InternalRedirect() ) {
                NGX_BROKER_MODULE_DEBUG_LOG(ngx_http_casper_broker_cdn_module, a_r, "cdn_module",
                                            "RW", "LEAVING",
    ...
    if ( nullptr == module ) {
        ...
        rv = ngx::casper::broker::cdn::Module::Factory(a_r, /* a_at_rewrite_handler */true);
        if ( NGX_OK == rv ) {
            rv = tackle_response(a_r, (ngx::casper::broker::cdn::Module*) ngx_http_get_module_ctx(a_r, ngx_http_casper_broker_cdn_module));
        } else if ( NGX_DONE == rv ) {

 ...
 * @return           A NGX code.
 */
ngx_int_t ngx::casper::broker::cdn::common::Module::ContentPhaseTackleResponse (ngx_http_request_t* a_r, ngx_module_t& a_module_t,
                                                                                const char* const a_token)
{
    ngx::casper::broker::cdn::common::Module* module = (ngx::casper::broker::cdn::common::Module*) ngx_http_get_module_ctx(a_r, a_module_t);
    if ( nullptr == module ) {
        // ... wtf?
        return NGX_ERROR;
    }
 ...
 * @return           A NGX code.
 */
ngx_int_t ngx::casper::broker::cdn::common::Module::RewritePhaseTackleResponse (ngx_http_request_t* a_r, ngx_module_t& a_module_t,
                                                                                const char* const a_token,
                                                                                const std::function<ngx_int_t()> a_factory)
{
    ...
    const auto tackle_response = [a_token] (ngx::casper::broker::cdn::common::Module* a_module) -> ngx_int_t {
    ...
    if ( nullptr == module ) {
        ...
        rv = a_factory();
        if ( NGX_OK == rv ) {
            rv = tackle_response((ngx::casper::broker::cdn::common::Module*) ngx_http_get_module_ctx(a_r, a_module_t));
        } else if ( NGX_DONE == rv ) {
